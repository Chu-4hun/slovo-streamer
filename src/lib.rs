...
/// Результаты запросов к трём платформам
...
/// Получает количество зрителей для всех трёх платформ параллельно.
...
pub async fn fetch_viewer_counts(
    ...
    let (twitch_res, kick_res, vk_res) = tokio::join!(twitch_fut, kick_fut, vk_fut);
